...
const quizCarousel = (
  carouselItems
) => `<div class="container-fluid bg-light rounded-3 p-3 d-flex justify-content-center align-self-center">
...
function carouselItem(qrr) {
  ...
  qrr.forEach((element, index) => {
    items += `<div class="carousel-item px-5"> <!--active-->
        <div class="que p-5">
            <div class="col text-center fs-6 mb-1 fw-bold">${index + 1}/${
      qrr.length
    }</div>
            <div class="row mb-5">
            ...
            <div class="row">
                <label class="col mx-2 my-2 rounded-3 form-check-label w-75" style="background-color: #dee2e6;" for="q${index}o1" >
                    <div class="form-check p-2 ms-4">
                        <input class="form-check-input" name="q${index}" type="radio" onclick="checkAns(${index}, '${
      element.options[0]
    }', 1)" id="q${index}o1" value="${element.options[0]}">
                ...
                <label class="col mx-2 my-2 rounded-3 form-check-label w-75" style="background-color: #dee2e6;" for="q${index}o2">
                    <div class="form-check p-2 ms-4">
                        <input class="form-check-input" type="radio" onclick="checkAns(${index}, '${
      element.options[1]
    }', 2)" name="q${index}" id="q${index}o2" value="${
      element.options[1]
    }" data-que-no=${index}>
            ...
            <div class="row">
                <label class="col mx-2 my-2 rounded-3 form-check-label w-75" style="background-color: #dee2e6;" for="q${index}o3">
                    <div class="form-check p-2 ms-4">
                        <input class="form-check-input" type="radio" onclick="checkAns(${index}, '${
      element.options[2]
    }', 3)" name="q${index}" id="q${index}o3" value="${
      element.options[2]
    }" data-que-no=${index}>
                ...
                <label class="col mx-2 my-2 rounded-3 form-check-label w-75" style="background-color: #dee2e6;" for="q${index}o4">
                    <div class="form-check p-2 ms-4">
                        <input class="form-check-input" type="radio" onclick="checkAns(${index}, '${
      element.options[3]
    }', 4)" name="q${index}" id="q${index}o4" value="${
      element.options[3]
    }" data-que-no=${index}>
...
durations.forEach((duration) => {
  if (typeof duration.questions == 'number') {
    $('#que-timing-dropdown').append(
      `<option value="${duration.questions}">${duration.questions} Questions / ${duration.timing} Minutes</option>`
    );
  } else {
    $('#que-timing-dropdown').append(
      `<option value="${duration.questions}">${duration.questions}</option>`
    );
  }
});
...
$('#quiz-form').on('change', () => {
  if (
    $('#difficulty-dropdown').val() != null &&
    $('#category-dropdown').val() != null &&
    $('#que-timing-dropdown').val() != null
  ) {
...
let queNumbers;
$('#que-timing-dropdown').on(
  'change',
  () => (queNumbers = $('#que-timing-dropdown').val())
);
...
$('#quiz-form').on('submit', (e) => {
  ...
  $('#submit-btn').html(
    `<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true" style="width: 3rem; height: 3rem; border-width: .4rem;"></span>`
  );
  ...
  getQA(
    $('#que-timing-dropdown').val(),
    $('#category-dropdown').val(),
    $('#difficulty-dropdown').val()
  );
});
...
const checkAns = (index, option, optionNo) => {
  if (option == qa[index].ans) {
    $(`input[id="q${index}o${optionNo}"]`)
      .parents()
      .find(`label[for="q${index}o${optionNo}"]`)
      .addClass('bg-success text-light fw-bold');
    $('#currentScore').text(Number($('#currentScore').text()) + 1);
  } else {
    $(`input[id="q${index}o${optionNo}"]`)
      .parents()
      .find(`label[for="q${index}o${optionNo}"]`)
      .addClass('bg-danger text-light fw-bold');
  }
  ...
  if (
    $('div[data-attempted=true]').length === Number($('#totalScore').text())
  ) {
    $('#result').html(`
            <span class="badge bg-success fs-4">YOU SCORED ${$(
              '#currentScore'
            ).text()} OUT OF ${$('#totalScore').text()}</span>
        `);
  }
};
